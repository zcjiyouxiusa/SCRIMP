...
class SCRIMPNet(nn.Module):
    ...
    def forward(self, obs, vector, input_state, message, obs_agents):
        ...
        c1 = torch.reshape(c1, (-1, num_agent, NetParameters.NET_SIZE))

        # print(f"after c1.shape:{c1.shape}")
        # print(f"c1.shape:{c1.shape}")
        # print(f"memories.shape:{memories.shape}") 
        # # print(f"h2.shape:{h2.shape}")
        # print(f"c1.device:{c1.device}")
        # print(f"memories.device:{memories.device}")
        # print(f"h2.device:{h2.device}")
